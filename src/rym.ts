...
export const fetchAlbumData = async (
  ...
  const data = await getPageHtml(browserPage, getRymUrl(result.releaseHref));

  const albumData = (await xray(data, ".release_page", {
...
export const loadRymAlbumData = async (
  ...
  logger.info(albumData, "Saved RYM album data");
};
